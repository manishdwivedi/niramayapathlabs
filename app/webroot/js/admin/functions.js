...
$(document).ready(function() {
	...
	$("#form_save_stl").submit(function(e){
		e.preventDefault();
		var remark = $('#sampleRemarks').val();
		var data = $("#form_save_stl").serialize();
		$('#loader').html('<span style="color:red;font-size: large;"> Kindly wait data is being processed.</div> ');
		$('#stl_btn').hide();
		$.ajax({
                    type: "POST", 		//GET or POST or PUT or DELETE verb
                    url: "/admin/samples/save_stl_data", 		// Location of the service
                    data: data,  //Data sent to server
                    dataType: "json",		
                    success: function (response) {//On Successful service call
                    	console.log(response);
						if(response["sc"]=="Success")
						{
							if(response["stl"]=="Success")
								$('#result_sample').html('<span style="color:green;font-size: large;"> Data Saved Successfully and Sent to Labmate.</div> ');
							else
								$('#result_sample').html('<span style="color:Red;font-size: large;"> Data Saved Successfully but Couldn\'t be Sent to Labmate</div> ');
						}
						else
						{
							$('#result_sample').html('<span style="color:Red;font-size: large;"> Error occurred During saving data.</div> ');
							$('#stl_btn').show();
						}
						$('#loader').html('');
                    },
...
{
	var sample_count = $('#sample_count').val();
	var remark = $('#sampleRemarks').val();
	var count = $('.radio:checked').length;
	if(remark=='')
		$( "#errorsample" ).html( '<span style="color:Red;font-size: large;">Remark cannot be empty.</div>' );
	if(count!=sample_count)
		$( "#errorsample" ).html( '<span style="color:Red;font-size: large;">Each sample should be marked with one of the status (Recieved,Rejected,SNR)</div>' );
	if(remark!='' && count==sample_count)
	{
		$( "#errorsample" ).html('');
		$("#saveSample").submit();
	}
}
function sampleSelect(id,e)
{
	var sample_count = $('#sample_count').val();
	var str = e.id.split('_');

	if(str[0]=='recieved')
		$('#samplestatus'+id).val('1');
	if(str[0]=='rejected')
		$('#samplestatus'+id).val('2');
	if(str[0]=='snr')
		$('#samplestatus'+id).val('3');
	
	$('.sampleSelect_'+id).attr('checked', false);
	$('#'+e.id).attr('checked', true);
	var total = $('.success:checked').length;

	if(total==0)
	{
		$('#sample_Status').val('Rejected');
		$('#sample_review_btn').hide();
		$('#rejectSample').show();
	}
	else if(total<sample_count)
	{
		$('#rejectSample').hide();
		$('#sample_review_btn').show();
		$('#sample_Status').val('Partial');
	}
	else{
		$('#rejectSample').hide();
		$('#sample_review_btn').show();
		$('#sample_Status').val('Success');
	}
}
