...
function new_patient()
{
	...
	//$('#MemberName').hide();
	//$('#UserList').hide();
	$('#InputMemberRelation').hide();
	$('#UserRelation').hide();
	$('#UserLandline').show();
	$('#InputMemberContact').show();
	$('#UserType').show();
	$('#InputMemberType').show();
}
...
$(function() {
	$('#HealthReference').keyup(function(){
		var reference = $('#HealthReference').val();
		$('#ref_load').show();

		jQuery.ajax({
			type:'GET',
			url:siteUrl+'admin/samples/search_reference?ref='+reference,
			dataType:'json',
			success:function(data){
				$('#ref_load').hide();
				if(data.status == 'failure')
				{
					$('#ref_error').html("Duplicate Reference Number. Kindly Enter New One.");
					$('#editupdate').hide();
				}
				else
				{
...
function add_test(val)
{
	jQuery.ajax({
		...
		success:function(data){
			...
			var testlist = $('#HealthTestId').val();
			var tests = testlist.replace(',','@');
			console.log(siteUrl+'admin/samples/get_doc?tests='+tests);
			jQuery.ajax({
				type:'GET',
				url:siteUrl+'admin/samples/get_doc?tests='+tests,
				success:function(data){
					$(data).insertAfter('#docs');
					//$('#docs').html()
					console.log(data);
				}
			});
...
function save_user()
{
	...
	document.getElementById("msg_relation").innerHTML="";
	document.getElementById("msg_user_type").innerHTML="";
	...
	if(document.form1.UserType.value=='' && $("#UserType").is(":visible"))
	{
		document.getElementById("msg_user_type").innerHTML="Please Select User Type";
		str=false;
	}

	if(document.form1.UserRelation.value=='' && $("#UserRelation").is(":visible"))
	{
		document.getElementById("msg_relation").innerHTML="Please Select Relation";
		str=false;
	}
	...
	if($("#UserLandline").is(":visible"))
	{
		if(isNaN(document.form1.UserLandline.value))
		{
			document.getElementById("msg5").innerHTML="Please Insert Numeric Mobile No.";
			str = false;
		}
		else if(document.form1.UserLandline.value.length<10)
		{
	...
	if($('#selectedUser').length && $("#selectedUser").is(":visible"))
	{
		parent_id = document.form1.selectedUser.value;
	}
	
	if($('#selectedSubUser').length && $("#selectedSubUser").is(":visible"))
	{
		sub_user = document.form1.selectedSubUser.value;
	}
	...
	if(str==true)
	{
		...
		jQuery.ajax({
			type:'POST',
			url:siteUrl+'admin/samples/create_subuser',
			dataType:'json',
			data : {
				type : document.form1.UserType.value,
				name : document.form1.UserName.value,
				age : document.form1.UserAge.value,
				gender : document.form1.UserGender.value,
				contact : document.form1.UserLandline.value,
				alt_contact : document.form1.UserAlternateContact.value,
				email : document.form1.UserEmail.value,
				alt_email : document.form1.UserAlternateEmail.value,
				pincode : document.form1.UserPincode.value,
				parent_id : parent_id,
				sub_user : sub_user,
				address1 : document.form1.UserAddress1.value,
				address2 : document.form1.UserAddress2.value,
				relation : document.form1.UserRelation.value,
				city_id : document.form1.UserCityId.value,
				state : document.form1.UserState.value,
				landmark : document.form1.UserLandmark.value
			},
			success:function(response){
				console.log(response);	
				if(response.user_info.success == 'success')
				{
					$('#UserListIssue').hide();
					$('#HealthUserId').val(response.user_info.user_id);
					$('#PatientInfo').hide();	
					...
					$('#UserList').hide();
					$('#SubUserList').hide();
					$('#HealthSameAddress1').val(document.form1.UserAddress1.value);
					$('#HealthSameAddress2').val(document.form1.UserAddress2.value);
					$('#HealthCity').val(document.form1.UserCityId.value);
					$('#HealthStateId').val(document.form1.UserState.value);
					$('#HealthPincodeId').val(document.form1.UserPincode.value);
					$('#HealthLandmark').val(document.form1.Landmark.value);
			        ...
			        $('#HealthMedicalReferenceNumber').val(response.user_info.user_id);
			        $('#HealthReference').val(response.user_info.user_id+'-'+refdatetime);
				}

				if(response.user_info.success == 'notsuccess')
				{
					console.log(response);
				}
			}
...
{
	var thisCheck = $('#selectedSubUser').val();
	jQuery.ajax({
		type:'GET',
		url:siteUrl+'admin/samples/get_user_detail?user_id='+thisCheck,
		dataType:'json',
		success:function(data){
			var currentdate = new Date(); 
			var refdatetime = currentdate.getDate() + ""
			...
			$('#HealthUserId').val(thisCheck);
			$('#HealthMedicalReferenceNumber').val(thisCheck);
			$('#HealthReference').val(thisCheck+'-'+refdatetime);
			...
			$('#HealthSameAddress1').val(data.user_info.add_line1);
			$('#HealthSameAddress2').val(data.user_info.add_line1);
			$('#HealthCity option[value='+data.user_info.user_data.User.city+']').attr('selected','selected');						
			$('#HealthStateId option[value='+data.user_info.user_data.User.state+']').attr('selected','selected');
			
			$('#HealthPincodeId').val(data.user_info.user_data.User.pincode);
			$('#HealthLandmark').val(data.user_info.user_data.User.landmark);
		}
	});
}

function check_subuser()
{
	var thisCheck = $('#selectedSubUser').val();
	//console.log(thisCheck);

	if(thisCheck=="new_sub_patient")
	{
		$('#continue_booking').hide();
		$('#PatientInfo').show();
		$('#UserLandline').hide();
		$('#InputMemberContact').hide();
		$('#InputMemberType').hide();
		$('#UserType').hide();
	}
	else
	{
...
function get_subuser(id)
{
	var thisCheck = $('#selectedUser').val();
	//console.log(thisCheck);
	...
	if(thisCheck!="")
	{
		jQuery.ajax({
			type:'GET',
			url:siteUrl+'admin/samples/search_subuser?req_val='+thisCheck,
			dataType:'json',
			success:function(data){
				console.log(data);
				if(data.user_info.success == 'success')
				{
					...
					if(data.p_user_type == 'individual')
					{
						if(data.user_info.user_list.length != 0)
						{
							rep_div +='<td width="15%" class="boldText">Sub User List</td>';
							rep_div +='<td>';
							rep_div +='<select border="0" width="100%" id="selectedSubUser" onchange="check_subuser()" class="input-text" name="sub_user">';
							rep_div +='<option value="0">Select Sub User</option>';
							...
							jQuery.each(data.user_info.user_list,function(index, value)
							{
								...
								rep_div +='<option value="'+value.User.id+'">'+value.User.name+' - '+value.User.alternate_contact+' - '+gender+'/'+value.User.age+'</option>';
							});
							rep_div +='<option value="new_sub_patient">Add New Sub Patient</option>';
					...
					{
						rep_div +='<td width="15%" class="boldText">Corporate Sub Userlist</td>';
						rep_div +='<td>';
						rep_div +='<input class="input-text" type="text" id="search_cop_user" onkeyup="search_corporate()" placeholder="Enter Atleast 4 characters">';
						rep_div +='</td>';
						rep_div1 +='<td width="15%" class="boldText"></td>';
						rep_div1 +='<td>';
						rep_div1 +='<select border="0" width="100%" id="selectedSubUser" onchange="check_subuser()" class="input-text" name="sub_user"></select>';
						rep_div1 +='</td>';

					}

					$('#InputMemberRelation').show();
					$('#UserRelation').show();
					$('#CorporateSubUserList').show();
					$('#SubUserList').show();
					$('#CorporateSubUserList').html(rep_div1);
					$('#SubUserList').html(rep_div);
					$('#SearchSubUserImg').hide();
					$('#PatientInfo').hide();
					$('#PatientOtherInfo').hide();
					$('#UserListIssue').hide();
				}
			},
			beforeSend:function(){
				jQuery('#SearchSubUserImg').show();
			},
		});
	}
}
...
{
	var search_val = $('#search_cop_user').val();
	var search_contact = $('#selectedUser').val();

	if(search_val.length > 4)
	{
		var dataString = 'user_id='+search_contact+'&name='+search_val;
		jQuery.ajax({
			type:'POST',
			url:siteUrl+"admin/samples/get_corporate_user_detail",
			data: dataString,
			cache: false,
			success:function(data){
				$("#selectedSubUser").html(data);
			}
		});	
	}
}
...
function search_user()
{
	...
	//alert(siteUrl+'admin/samples/search_user?req_val='+search_val);
	jQuery.ajax({
		type:'GET',
		...
		success:function(data){
			...
			if(data.user_info.success == 'success')
			{
				...
				if(data.user_info.user_list.length != 0)
				{
					rep_div +='<td width="15%" class="boldText">User List</td>';
					rep_div +='<td>';
					rep_div +='<select border="0" width="100%" name="parent_user" id="selectedUser" onchange="get_subuser()" class="input-text">';
					rep_div +='<option value="0">Select User</option>';
					...
					jQuery.each(data.user_info.user_list,function(index, value)
					{
						...
						rep_div +='<option value="'+value.id+'">'+value.name+' - '+value.contact+' - '+gender+'/'+value.age+'</option>';
					});

					rep_div +='</select>';
					rep_div += '&nbsp;&nbsp;&nbsp;&nbsp;<img src="/img/frontend/loading.gif" width="40" height="40" style="margin: 0px 0px -15px; display: none;" id="SearchSubUserImg" alt="">';
					rep_div +='</td>';
					$('#UserList').show();
					...
					$('#PatientOtherInfo').hide();
					$('#UserListIssue').hide();
					$('#CorporateSubUserList').hide();
					$('#SubUserList').hide();

					if(data.user_info.p_type=='child')
					{
						var rep_div2 = '';
						var rep_div1 = '';
						...
						if(data.p_user_type == 'individual')
						{
							if(data.user_info.subuser_list.length != 0)
							{
								rep_div2 +='<td width="15%" class="boldText">Sub User List</td>';
								rep_div2 +='<td>';
								rep_div2 +='<select border="0" width="100%" id="selectedSubUser" onchange="check_subuser()" class="input-text" name="sub_user">';
								rep_div2 +='<option value="0">Select Sub User</option>';
								
								jQuery.each(data.user_info.subuser_list,function(index, value)
								{
									var gender = 'F';
									if(value.User.gender==1)
									{
									...
									rep_div2 +='<option value="'+value.User.id+'">'+value.User.name+' - '+value.User.contact+' - '+gender+'/'+value.User.age+'</option>';
								});
								rep_div2 +='<option value="new_sub_patient">Add New Sub Patient</option>';
						...
						{
							rep_div2 +='<td width="15%" class="boldText">Corporate Sub Userlist</td>';
							rep_div2 +='<td>';
							rep_div2 +='<input class="input-text" type="text" id="search_cop_user" onkeyup="search_corporate()" placeholder="Enter Atleast 4 characters">';
							rep_div2 +='</td>';
							rep_div1 +='<td width="15%" class="boldText"></td>';
							rep_div1 +='<td>';
							rep_div1 +='<select border="0" width="100%" id="selectedSubUser" onchange="check_subuser()" class="input-text" name="sub_user"></select>';
						...
						$('#selectedUser').val(data.user_info.parent_id);
						$('#InputMemberRelation').show();
						$('#UserRelation').show();
						$('#CorporateSubUserList').show();
						$('#SubUserList').show();
						$('#CorporateSubUserList').html(rep_div1);
						$('#SubUserList').html(rep_div2);
						$('#SearchSubUserImg').hide();
						$('#PatientInfo').hide();
						$('#PatientOtherInfo').hide();
						$('#UserListIssue').hide();
...
function getcitystate()
{
	...
	if(pin.length==6)
	{
		...
		jQuery.ajax({
			...
			success: function(response) {
			console.log(response);
				$('#UserCityId option[value='+response["city"]+']').attr('selected','selected');						
...
function checkphone(tag)
{
	...
	if(phone.val().length==10)
	{
		jQuery.ajax({
			type:'GET',
			url:siteUrl+'admin/samples/check_phone?contact='+phone.val(),
			success: function(response) {
				console.log(response.success);
				if(response.success=='Failure'){
					$('.'+tag.id+'Error').html("Contact Already Exist");
					$('.'+tag.id+'Error').show();
					$('#submit_user').hide();
				}
				else
...
<div class="contentcontainer">
	...
	<table border="0" cellspacing="2" cellpadding="0" align="center" width="100%">
		...
		<tr id="SearchExistPatient">
			<td colspan="2" style="font-weight:bold; text-decoration:underline; background-color:#999999; color:#FFFFFF; text-align:center;">Search Existing Patient/Member</td>
		</tr>
		<tr id="MemberName">
			<td width="15%" class="boldText">Existing Patient/Member</td>
			<td>
				<?php echo $form->text('Health.user',array('class'=>'input-text'));?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" onclick="search_user();" style="color:#0066FF;">Search?</a>&nbsp;&nbsp;&nbsp;(Search By : Phone)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $html->image('frontend/loading.gif',array('width'=>40,'height'=>40,'style'=>'margin:0 0 -15px; display:none;','id'=>'SearchUserImg'));?>
			</td>
		</tr>
		<tr id="UserList" style="display:none;"></tr>
		<tr id="UserListIssue" style="display:none;"></tr>
		<tr id="SubUserList" style="display:none;"></tr>
		<tr id="CorporateSubUserList" style="display:none;"></tr>
		<tr id="continue_booking" style="display:none;">
			<td>
				<input type="button" value="Continue" name="Continue" class="btn" onclick="continue_booking();"> 
			</td>
		</tr>
	</table>

	<table border="0" cellspacing="2" cellpadding="0" align="center" width="100%" id="PatientInfo" style="display:none;">
		<tr>
			<td colspan="2"><hr></hr></td>
		</tr>
		<tr>
			<td colspan="2" style="text-align:center;color: #859C27;"><h2><b>Patient Information</b></h2></td>
		</tr>
		<tr id="InputMemberType">
			<td width="15%" class="boldText">Patient Type<font color="#FF0000">*</font></td>
			<td>
				<select name="data[User][type]" id="UserType" class="input-text" "required">
					<option value="">Select Type</option>
					<option value="individual">Individual</option>
					<option value="corporate">Corporate</option>
				</select>
				<div id="msg_user_type" style="color:#FF0000; font-size:12px;"></div>
			</td>
		</tr>
		<tr id="InputMemberRelation">
			<td width="15%" class="boldText">Relation with Patient <font color="#FF0000">*</font></td>
			<td>
				<select name="data[User][relation]" id="UserRelation" class="input-text" "required">
					<option value="">Select Relation</option>
					<?php foreach($relation as $key => $val) {?>
					<option value="<?php echo $key;?>"><?php echo $val;?></option>
					<?php }?>
				</select>
				<div id="msg_relation" style="color:#FF0000; font-size:12px;"></div>
			</td>
		</tr>
		<tr id="InputMemberName">
			<td width="15%" class="boldText">Patient Name <font color="#FF0000">*</font></td>
			<td>
				<?php echo $form->text('User.name', array('class'=>'input-text','required')); ?>
		...
		<tr id="InputMemberGender">
			...
			<td>
				<select name="data[User][gender]" id="UserGender" class="input-text" "required">
		...
		<tr id="InputMemberAge">
			...
			<td>
				<?php echo $form->text('User.age', array('class'=>'input-text','style'=>'width:50px;','required')); ?>
				<div id="msg3" style="color:#FF0000; font-size:12px;"></div>
			</td>
		</tr>
		
		<tr id="InputMemberContact">
			<td width="15%" class="boldText">Registered Number <font color="#FF0000">*</font></td>
			<td>
				<?php echo $form->text('User.landline', array('class'=>'input-text phone','maxlength'=>'10','minlength'=>'10','onkeypress'=>'return checknum(event)','onkeyup'=>'checkphone(this)','required')); ?>
				<?php echo $html->image('frontend/loading.gif',array('width'=>40,'height'=>40,'style'=>'margin:0 0 -15px; display:none;','id'=>'UserLandlineImg'));?>
				<div id="msg5" class="UserLandlineError" style="color:#FF0000; font-size:12px;"></div>
			</td>
		</tr>
		...
		<tr id="InputMemberAltContact">
			...
			<td>
				<?php echo $form->text('User.alternate_contact', array('class'=>'input-text phone','maxlength'=>'10','minlength'=>'10','onkeypress'=>'return checknum(event)')); ?>
			</td>
		</tr>
		...
		<tr id="InputMemberEmail">
			...
			<td>
				<?php echo $form->text('User.email', array('class'=>'input-text')); ?>
		...
		<tr id="InputMemberAltEmail">
			...
			<td>
				<?php echo $form->text('User.alternate_email', array('class'=>'input-text')); ?>
				<!--<div id="msg6" style="color:#FF0000; font-size:12px;"></div>-->
			</td>
		</tr>
		...
		<tr id="InputMemberAddress1">
			...
			<td>
				<?php echo $form->text('User.address1', array('class'=>'input-text','required')); ?>
			</td>
		</tr>
		<tr id="InputMemberAddress2">
			<td width="15%" class="boldText">&nbsp;</td>
			<td>
				<?php echo $form->text('User.address2', array('class'=>'input-text')); ?>
		...
		<tr id="InputMemberPincode">
			...
			<td>
				<?php echo $form->text('User.pincode', array('class'=>'input-text','onkeyup'=>'getcitystate();','required')); ?>
		...
		<!--<tr id="InputMemberLocality">
			...
			<td>
				<select name="data[User][locality]" id="HealthLocality" class="input-text">
				</select>
		...
		<tr id="InputMemberCity">
			...
			<td>
				<select name="data[User][city_id]" id="UserCityId" class="input-text" readonly>
		...
		<tr id="InputMemberState">
			...
			<td>
				<select name="data[User][state]" id="UserState" class="input-text" readonly>
		...
		<tr id="InputMemberLandmark">
			...
			<td>
				<?php echo $form->text('User.landmark', array('class'=>'input-text')); ?>
		...
		<tr id="submit_user">
			<td width="15%">&nbsp;</td>
			<td>
				<input type="button" id="save_info" class="btn" value="Save" name="Save" onclick="save_user();"/>
				<!--<?php echo $form->submit('Save', array('div'=>false, 'class' => 'btn','id'=>'editupdate')); ?>-->
				<a onclick="cancel_new_user();" class="btn" style="color:#FFFFFF;">Cancel</a>
			</td>
		</tr>	
	</table>
	...
	-->
	<!--Code For Inserting BMI value Ends-->
	<table border="0" cellspacing="2" cellpadding="0" align="center" width="100%" id="PatientOtherInfo" style="display:none;">
		<tr>
			<td><?php echo $form->hidden('Health.user_id');?></td>
		</tr>
		<!--booking on behalf of other-->
        <?php $admin_u_type = $session->read('Admin.userType');
		...
		<tr id="InputMemberReference">
	        ...
	        <td>
	            ...
	            <div id="ref_error" style="color:#FF0000; font-size:12px;"></div>
	            <img id="ref_load" src="/img/frontend/loading.gif" style="top:190px; left:265px;display:none;" alt="">
	        </td>
	    ...
	    <tr id="add_line1">
	    	...
	    	<td>
	    		<input type="text" id="HealthSameAddress1" name="data[Health][same_address1]" class="input-text">
	    	</td>
	    </tr>
		<tr id="add_line2">
			<td width="15%" class="boldText">&nbsp;</td>
	    	<td>
	    		<input type="text" id="HealthSameAddress2" name="data[Health][same_address2]" class="input-text">
	    	</td>
		</tr>
		<tr id="pincode">
			<td width="15%" class="boldText">Pincode</td>
	    	<td>
	    		<input type="text" id="HealthPincodeId" name="data[Health][same_pincode]" class="input-text" onkeyup="getcitystate1()">
	    	</td>
		</tr>	
		<tr id="city">
			<td width="15%" class="boldText">City</td>
			<td>
				<select name="data[Health][same_city]" id="HealthCity" class="input-text">
					<option value="">Select City</option>
					<?php foreach($city as $key => $val) {?>
					<option value="<?php echo $val['City']['id'];?>"><?php echo $val['City']['name'];?></option>
					<?php }?>
		...
		<tr id="state">
			<td width="15%" class="boldText">State</td>
			<td>
				<select name="data[Health][same_state]" id="HealthStateId" class="input-text">
					<option value="">Select State</option>
					<?php foreach($state as $key => $val) {?>
					<option value="<?php echo $val['State']['id'];?>"><?php echo $val['State']['name'];?></option>
					<?php }?>
		...
		<tr id="landmark">
			<td width="15%" class="boldText">Landmark</td>
			<td>
				<input type="text" id="HealthSameLandmark" name="data[Health][same_landmark]" class="input-text">
			</td>
		</tr>
