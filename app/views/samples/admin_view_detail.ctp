...
function reschedule()
{
	...
	else
	{
		...
		$('#HealthSampleDate1').prop('disabled',true);
	}
}
...
function partial_closed_stat()
{
	$('.tabs').hide();
    $('#PartialMarkClosed').show();
}
...
<script type="text/javascript">
$(function() {
	//$(".sample_time").attr("min","06:00:00");
	//$(".sample_time").attr("max","18:00:00");

	$(".sample_time").wickedpicker({twentyFour: true});
...
function delete_single_test(val1,val2)
{
	...
	var r = confirm("Are you sure, You want to remove Test?");

	if (r == true) {
		var dis_amt = document.getElementById('HealthDiscountAmount').value;
		var disc_code = document.getElementById('HealthDiscountCode').value;
			...
			success:function(data){
				if(data.test_info.success == 'success')
				{
					...
					if(data.test_info.new_tests.length != 0)
					{
						...
						if(data.test_info.total_amt<0)
						{
							alert("As the Total amount is less than Discount Amount, Discount has been rempved from this Order.");
							$('#HealthDiscountAmount').val("0");
							$('#HealthDiscountCode').val("");
							$('#HealthDiscountAmountReason').val("");
						}
			...
			success:function(data){
				if(data.test_info.success == 'success')
				{
					...
					if(data.test_info.new_tests.length != 0)
					{
						...
						if(data.test_info.total_amt<dis_amt)
						{
							alert("As the Total amount is less than Discount Amount, Discount has been rempved from this Order.");
							$('#HealthDiscountAmount').val("0");
							$('#HealthDiscountCode').val("");
...
<table border="0" cellspacing="2" cellpadding="0" align="center" width="100%">
	...
	<?php if(isset($doc_list)){?>
		<tr>
			<td width="15%" class="boldText" colspan="2" style="text-decoration:underline;">Documents Submitted</td>
		</tr>
		<?php foreach($doc_list as $val) {?>
			<tr>
				<td width="15%" class="boldText"><?php echo $val['name']; ?></td>
				<td><?php if($val['type']=="doc")
				{
					echo "<a href='".urldecode($val['value'])."' target='_blank'>View Doc</a>";
				} 
				else
				{
					echo $val['value'];
	...
	<?php if($this->data['Health']['balance_amt']>0) { ?>
		<tr>
			<td colspan='2'>
				<input style="width:100px;" class="input-text phone" name="payment_number" value="<?php echo $this->data['Health']['landline'];?>" id="payment_number" type="text" maxlength="10" minlength="10">
				<input type="button" id="pay_message" onclick="payment_message()" class="btn" value="Send Payment Link On SMS">
				
				<label id="pay_error" style="display:none;color:red;font-size:large;">Invalid Phone Number.</label>
				<label id="pay_processing" style="display:none;color:red;font-size:large;">Payment Link Is being Sent.</label>
				<label id="pay_success" style="display:none;color:green;font-size:large;">Payment Link Sent Using SMS.</label>
			</td>
		</tr>
	<?php } ?>
	...
	<tr>
		...
		?>
		<?php }?>
		<?php if($val['Paytrack']['pay_mode'] == 'wallet'){?>
		<?php 
		$pay_mode = 'wallet';
		?>
		<?php }?>
		...
		?>
		<?php if($val['Paytrack']['pay_mode'] == 'voucher'){
			$pay_mode = 'Voucher Redeemed';
		}
		?>
		...
		<td>
			...
			<?php }?>
			<?php if($val['Paytrack']['pay_mode'] == 'wallet'){?>
			<?php echo 'Rs. '.$val['Paytrack']['pay_install'].' received through '.$val['Paytrack']['org_name'].' by '.$pay_mode.' on '.date('d-m-Y',strtotime($val['Paytrack']['admin_receive_date']));?>
			<?php }?>
			<?php if($val['Paytrack']['pay_mode'] == 'cash'){?>
            ...
            <?php }?>
			<?php if($val['Paytrack']['pay_mode'] == 'voucher'){?>
			<?php echo 'Rs. '.$val['Paytrack']['pay_install'].' received by Redeeming Voucher';?>
                <?php echo ' ( '. preg_replace("/(?!^).(?!$)/", "*", $val['Paytrack']['remarks']). ' ) on '.date('d-m-Y',strtotime($val['Paytrack']['admin_receive_date'])); ?>
			<?php }?>
		</td>
	</tr>
	<?php $k++;}?>
	...
	<tr>
		...
		<td>
			<?php 
			if(!empty($this->data['Health']['prescription_url'])){
				$urls = explode("@@@@@",$this->data['Health']['prescription_url']);
				$count = 1;
				foreach($urls as $key=>$val)
				{ 
					if (strpos($val, 'https:') !== false) { 
						echo "<a href='".$val."' target='_blank'>Prescription ".$count."</a><br><br>";
					}
					else
					{
	...
	<?php if($this->data['Health']['assigned_lab']=='121' && ($this->data['Health']['requ_status']=='0' || $this->data['Health']['requ_status']=='15')) { ?>
		<!--<tr>
			<td colspan="2">
				<span id="dropletprocessing" style="color:Red;display:none;font-size: x-large;">The Data is being processed.</span>
				<span id="dropletcompleted" style="color:Green;display:none;font-size: x-large;">Droplet Order successfully Placed.</span>
				<span id="dropletfailed" style="color:Red;display:none;font-size: x-large;">Unable to Place Order.</span>
				<br>
				<input id="dropletapi" class="btn" type="button" onclick="droplet_trigger(<?php echo $this->data['Health']['id'];?>)" value="Droplet Order"/>
			</td>
		</tr>-->
	<?php } ?>
	...
	<script>
		function payment_message()
		{
			...
			if(phone!="")
			{
				...
				$.ajax({
					type:'POST',
					url:siteUrl+'admin/samples/pay_on_sms',
					data : {
						phone : phone,
						health_id : healthId,
					},
	...
	<tr>
		<td colspan="2">
			<?php
						...
						<?php if($this->data['Health']['requ_status'] == 0 || $this->data['Health']['requ_status'] == 15){
                            echo $form->submit('Confirm Booking', array('div'=>false, 'class' => 'btn','onclick'=>'confirm_booking();'));
                        } ?>&nbsp;&nbsp;&nbsp;
			...
			<?php  if(in_array($this->data['Health']['requ_status'],array('5','6','7','12')))
			{
				echo $form->submit('Send Report', array('div'=>false, 'class' => 'btn','onclick'=>'show_up_notify();')); 
			}
			?>&nbsp;&nbsp;&nbsp;
		...
		if($this->data['Health']['requ_status'] == 7) {?>
			<?php echo $form->submit('Mark as Partial Closed', array('div'=>false, 'class' => 'btn','onclick'=>'partial_closed_stat();')); ?>&nbsp;&nbsp;&nbsp;
		<?php 	} ?>
		...
		<?php echo $form->submit('Ticket Management', array('div'=>false, 'class' => 'btn','onclick'=>'show_ticket_mgt();')); ?>&nbsp;&nbsp;&nbsp;
		<?php echo $form->submit('Specimen Detail', array('div'=>false, 'class' => 'btn','onclick'=>'specimen_detail();')); ?>&nbsp;&nbsp;&nbsp;
		<?php echo $form->submit('Json Log', array('div'=>false, 'class' => 'btn','onclick'=>'json_log();')); echo "&nbsp;&nbsp;&nbsp";?>
		<?php  echo $form->submit('Patient Doc Upload', array('div'=>false, 'class' => 'btn','onclick'=>'patient_doc();')); echo "&nbsp;&nbsp;&nbsp"; ?>

		<!--<?php  if($user_type == 'A') { echo $form->submit('Cancel Request', array('div'=>false, 'class' => 'btn','onclick'=>'cancel_request();')); echo "&nbsp;&nbsp;&nbsp";} ?>-->
		<!--<?php  if($user_type == 'A') { echo $form->submit('Reject Request', array('div'=>false, 'class' => 'btn','onclick'=>'reject_request();')); echo "&nbsp;&nbsp;&nbsp";} ?>-->
	...
	function check_barcode(e,id,item_id)
	{
		var b_id = e.id;
		var b_value = e.value;
		var h_id = id;
		...
		$.ajax({
			type:'POST',
			url:siteUrl+'admin/samples/check_barcode',
			data : {
				id : h_id,
				bcode : b_value, 
			},
			...
			success:function(result){
				...
				{	
					$('#barcode_issue_'+item_id).html("Duplicate Barcode Id in Different Order");
					$('#sample_check_'+item_id).val(1);
				}
				else
				{	
					$('#barcode_issue_'+item_id).html("");
					$('#sample_check_'+item_id).val(0);
				}

				var texts= $(".sample_checklist").map(function() {
				   return $(this).val();
				}).get();

				if(jQuery.inArray( "1", texts)>0)
				{
					$('#autosenttolab').hide();
					$('#stl_btn').hide();
				}
	...
	<tr id="PatientVital" class="tabs" style="display:none;">
		<td colspan="2">
	...
	<tr id="ticket_list" class="tabs" style="display:none;">
		<td colspan="2">
			<table style="width:100%;">
				<tr>
					<td class="headings altheading" style="border:0;" colspan="9"><h2>Ticket List</h2></td>
				</tr>
				<?php if(!empty($ticket)){?>
					<tr>
					...
					<?php $ctr=0; foreach($ticket as $val) {
						$class = "";
						if(($ctr%2) == 1){
							$class = " class=\"alt\"";
						}?>
						...
						<tr <?php echo $class;?>>
							...
							<td <?php echo $class;?>><?php echo $val['Ticket']['title'];?></td>
							<td <?php echo $class;?>><?php echo date('d-m-Y',strtotime($val['Ticket']['date']));?></td>
							<td <?php echo $class;?>><?php echo $category[$val['Ticket']['category']];?></td>
							<td <?php echo $class;?>><?php echo $users[$val['Ticket']['created_by']];?></td>
							<td <?php echo $class;?>><?php echo $priority[$val['Ticket']['priority']];?></td>
							<td <?php echo $class;?>><?php echo $users[$val['Ticket']['assigned_to']];?></td>
							<td <?php echo $class;?>><?php echo $status[$val['Ticket']['status']];?></td>
							<td <?php echo $class;?> style="text-align:center;"><?php echo $html->link('Show',array('controller'=>'ticket','action'=>'edit_ticket',base64_encode($val['Ticket']['id'])),array('target'=>'_blank'));?></td>
						</tr>
					<?php $ctr++; } ?>
				<?php } else { ?>
					<tr>
	...
	<tr id="notify" class="tabs" style="display:none;">
		<td colspan="2">
			<table style="width:100%;">
				<tr>
					<td class="headings altheading" style="border:0;" colspan="4"><h2>Send Report</h2></td>
				</tr>
				...
				<?php if($this->data['Health']['requ_status'] == '6' || $this->data['Health']['requ_status']=='7') { ?>
					<tr>
						<td colspan='2'>
							<input style="width:100px;" class="input-text email" name="report_email" value="<?php echo $this->data['Health']['email']; ?>" id="report_email" type="text" required>
							<?php echo $form->submit('Email Report To Patient', array('div'=>false, 'class' => 'btn','onclick'=>'return sendmailtopcc(this);','id'=>'emailtopcc')); ?>
							<span id="mail_issue" style="color:Red;display:none;font-size: x-large;"></span>
				...
				<?php if($this->data['Health']['requ_status'] == '6') { ?>
					<tr>
						<td colspan='2'>
							<input style="width:100px;background: lightgrey;" class="input-text phone" name="whatsapp_number" value="" id="whatsapp_number" type="text" maxlength="10" minlength="10" disabled>
							<input type="button" id="send_whatsapp" onclick="whatsapp()" class="btn" value="Send Report On Whatsapp" disabled>
							
							<label id="what_error" style="display:none;color:red;font-size:large;">Invalid Phone Number.</label>
							<label id="what_processing" style="display:none;color:red;font-size:large;">Report Is being Sent.</label>
							<label id="what_success" style="display:none;color:green;font-size:large;">Report Sent Using WhatsApp.</label>
						</td>
					</tr>

					<tr>
						<td colspan='2'>
							<input style="width:100px;" class="input-text phone" name="simple_number" value="" id="simple_number" type="text" maxlength="10" minlength="10">
							<input type="button" id="send_message" onclick="report_message()" class="btn" value="Send Report On SMS">
							
							<label id="sim_error" style="display:none;color:red;font-size:large;">Invalid Phone Number.</label>
							<label id="sim_processing" style="display:none;color:red;font-size:large;">Report Is being Sent.</label>
							<label id="sim_success" style="display:none;color:green;font-size:large;">Report Sent Using SMS.</label>
						</td>
					</tr>
				<?php } ?>
			</table>
	...
	<!--Code For Confirming Booking Starts Here -->
	<tr id="confirmBooking" class="tabs" style="display:none;">
		<td colspan="2">
			<?php echo $form->create(array('url'=>'/admin/samples/confirm_booking/'.base64_encode($this->data['Health']['id']),'id'=>'form_booking','name'=>'form_booking'));?>
			
			<?php echo $form->hidden('Health.login_action',array('value'=>'Super'));?>
			<table border="0" width="100%">
				<tr>
					<td class="headings altheading" style="border:0;" colspan="4"><h2>Confirmation Checklist</h2></td>
				</tr>
				
				<tr>
					<td style="font-weight:bold; width:100px;">Details</td>
					<td style="width:135px;" class="boldText">Status</td>
				</tr>
				<tr>
					<td style="width:100px;">Patient Details</td>
					<td style="width:135px;" class="boldText"><input type="checkbox" id="checkbox_pdetails" name="data[ConfirmBooking][patient_details]"></td>
				</tr>
				<tr>
					<td style="width:100px;">Contact Details</td>
					<td style="width:135px;" class="boldText"><input type="checkbox" id="checkbox_cdetails" name="data[ConfirmBooking][contact_details]" ></td>
				</tr>
				<tr>
					<td style="width:100px;">Test Details</td>
					<td style="width:135px;" class="boldText"><input type="checkbox" id="checkbox_pdetails" name="data[ConfirmBooking][test_details]" ></td>
				</tr>
				<tr>
					<td style="width:100px;">Payment Details</td>
					<td style="width:135px;" class="boldText"><input type="checkbox" id="checkbox_paydetails" name="data[ConfirmBooking][payment_details]" ></td>
				</tr>
				<tr>
					<td style="width:100px;">Collection Date & Time</td>
					<td style="width:135px;" class="boldText"><input type="checkbox" id="checkbox_cdatetime" name="data[ConfirmBooking][collection_date_time]" ></td>
				</tr>
				<tr>
					<td style="width:100px;">Sample Collection address</td>
					<td style="width:135px;" class="boldText"><input type="checkbox" id="checkbox_sample_c_address" name="data[ConfirmBooking][sample_collection_address]" ></td>
				</tr>
				<tr id="MakeDiv">
					<td style="width:100px;">Special Remarks If any</td>
					<td class="boldText">
						<textarea class="input-text" rows="5" cols="50" style="font-size:12px;" name="data[ConfirmBooking][remarks]" id="confirm_remarks" required></textarea>
					</td>
				</tr>
				<tr>
					<td>&nbsp;</td>
	...
	<!--Code For Confirming Booking Ends Here -->	
	
	<tr id="ActiveLog" class="tabs" <?php if(!isset($showActivity) || $showActivity != '1') { ?> style="display:none;" <?php } ?> >
		<td colspan="2">
	...
	<tr id="JsonLog" class="tabs" <?php if(!isset($showActivity) || $showActivity != '1') { ?> style="display:none;" <?php } ?> >
		<td colspan="2">
		...
		<th width="30%"><div style="text-align:center;">Response data.</div></th></tr>
				...
				if(($count%2) == 1){ $class = " class=\"alt\"";}else{ $class = ""; }?>
					<tr <?php echo $class;?>>
						...
						<td width="30%" style="text-align:center; border-left:1px solid #D8D8D8; border-right:1px solid #D8D8D8; border-bottom:1px solid #D8D8D8;">
						<?php  if(strpos($act['JsonData']['response_data'], "<html>") === false){ echo "<span class='boldText'>".$act['JsonData']['response_data']."</span>"; } else
						{ echo "-"; } ?>
						</td>
					</tr>
				<?php $count ++;}
	...
	<tr id="CxlRequest" class="tabs" style="display:none;">
		<td colspan="2">
			<?php echo $form->create(array('url'=>'/admin/samples/cxl_request/'.base64_encode($this->data['Health']['id']),'id'=>'form14','name'=>'form14'));?>
			<?php echo $form->hidden('Health.login_action',array('value'=>'Super'));?>
			<table border="0" width="100%">
				<tr>
					<td class="headings altheading" style="border:0;" colspan="4"><h2>Cxl Requested By Phlebo</h2></td>
				</tr>
				<tr>
					<td style="font-weight:bold; width:10px;"></td>
					<td style="width:135px;" class="boldText">Closed Reasons</td>
					<td style="width:55px;">
						<select id="closed_reason" name="data[Health][closed_reason]" required>
							<option>Select Reasons</option>
							<option value="1">Customer not available</option>
							<option value="2">In-correct address</option>
							<option value="3">Customer refused sample collection</option>
							<option value="4">Cancelled by Customer</option>
							<option value="5">Cancelled at Customer Point</option>
							<option value="6">Customer is not fasting</option>
							<option value="7">Phlebo couldn't contact customer</option>
				...
				<tr id="MessageDivCxl">
					<td>&nbsp;</td>
					<td class="boldText">Enter Comment</td>
					<td colspan="2"><?php echo $form->textarea('Health.lab_message_cxl',array('class'=>'input-text','rows'=>5,'cols'=>30,'style'=>'font-size:12px;','required'));?></td>
				</tr>
				<tr>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
	...
	<tr id="RejectRequest" class="tabs" style="display:none;">
		<td colspan="2">
			<?php echo $form->create(array('url'=>'/admin/samples/reject_request/'.base64_encode($this->data['Health']['id']),'id'=>'form13','name'=>'form13'));?>
			<?php echo $form->hidden('Health.login_action',array('value'=>'Super'));?>
			<table border="0" width="100%">
				<tr>
					<td class="headings altheading" style="border:0;" colspan="4"><h2>Rejection Requested </h2></td>
				</tr>
				<tr id="MessageDivRej">
					<td>&nbsp;</td>
					<td class="boldText">Enter Message</td>
					<td colspan="2"><?php echo $form->textarea('Health.lab_message_rej',array('class'=>'input-text','rows'=>5,'cols'=>30,'style'=>'font-size:12px;','required'));?></td>
				</tr>
				<tr>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
	...
	<tr id="LabMessage" class="tabs" style="display:none;">
		<td colspan="2">
			...
			<table border="0" width="100%">
				...
				{	
					...
					<tr <?php echo $class;?>>
						...
						<?php   echo "<span class='boldText' style='margin-left: 20px;'>".date('d-m-Y H:i:s',strtotime($message['LabMessageMaster']['date']))."</span>"; ?>
						</td>
	...
	<tr id="SentLab" class="tabs" style="display:none;">
		<td colspan="2">
	...
	<tr id="Samplesave" class="tabs" style="display:none;">
		<td colspan="2">
	...
	<tr id="patientDoc" style="display:none;">
		<td colspan="2">
			<?php echo $form->create(array('url'=>'/admin/samples/patient_doc/'.base64_encode($this->data['Health']['id']),'id'=>'saveSample','name'=>'saveSample','enctype'=>'multipart/form-data'));?>
			<?php echo $form->hidden('Health.request_id',array('value'=>$this->data['Health']['id']));?>
			<?php echo $form->hidden('Health.login_action',array('value'=>'Super'));?>
				<table border="0" width="100%">
					<tr>
						<td class="headings altheading" style="border:0;" colspan="4"><h2>Patient Document Upload</h2></td>
					</tr>
					<tr>
						<td style="font-weight:bold; width:10px;"></td>
						<td style="width:135px;" class="boldText">Patient Doc</td>
						<td style="width:55px;">
							<input type="file" name="patient_doc" id="patient_doc" required />					
						</td>
					</tr>
					<tr id="MessageDivCxl">
						<td>&nbsp;</td>
					...
					<tr>	
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td><?php echo $form->submit('Upload Doc', array('div'=>false, 'class' => 'btn')); ?></td>
						<td>&nbsp;</td>
					</tr>
	...
	<tr id="SentLabApi" class="tabs" style="display:none;">
		<td colspan="2">
			...
			<div style="padding: 10px;">
				...
				<div style="float:right;margin-right: 30px;">
					...
					<input class="sample_time" type="text" id="sample_collected_time" style="float:left;width: 90px;" name="data[Health][sample_collected_time]" value="<?php echo $this->data['Health']['sample_collected_time'];?>" required>
				</div>
			</div>
			<table border="0" width="100%" >
			...
			if(array_key_exists($val,$assigned_sample)) {
					...
					<td style="width:45%;">
						<input onkeyup="check_barcode(this,<?php echo $this->data['Health']['id']; ?>,<?php echo $val; ?>);" type="text" id="sample_barcode_<?php echo $val; ?>" name="sample_barcode[<?php echo $val; ?>]" value="<?php echo $value;?>" <?php echo $disabled;?>>

						<span style="color:red;" id="barcode_issue_<?php echo $val; ?>"></span>
						<input type="hidden" class="sample_checklist" id="sample_check_<?php echo $val; ?>" name="sample_check_<?php echo $val; ?>" value="0"/>
					</td>
				<?php }?>
				<tr style="background:red;color:black;font-weight: 600;">
					...
					<td>
						<input onkeyup="check_barcode(this,<?php echo $this->data['Health']['id']; ?>,<?php echo $val; ?>);" type="text" id="sample_barcode_others" name="sample_barcode_others" value="<?php echo $value;?>" <?php echo $disabled;?>>
						<span style="color:red;" id="barcode_issue_<?php echo $val; ?>"></span>
						<input type="hidden" class="sample_checklist" id="sample_check_<?php echo $val; ?>" name="sample_check_<?php echo $val; ?>" value="0"/>
					</td>
				</tr>
				<tr>
					<td>&nbsp;</td><td id="sample_collected_button" style="display:<?php /*if($this->data['Health']['requ_status'] >= 5 && $this->data['Health']['requ_status']<10) {*/ echo "inline";/* } */?>">
						<div id="loader"> </div>
						<input type="submit" id="stl_btn" class="btn" style="display:<?php echo $display;?>">
						<?php //echo $form->submit("Submit", array("div"=>false, "class" => "btn",'id'=>'stl_btn')); ?> 
	...
	<tr id="ProcessCancel" class="tabs" style="display:none;">
		<td colspan="2">
	...
	<tr id="raise_ticket" class="tabs" style="display:none;">
		<td colspan="2">
	...
	<tr id="specimen_d" class="tabs" style="display:none;">
		<td colspan="2">
	...
	<tr id="MarkClosed" class="tabs" style="display:none;">
		<td colspan="2">
			...
			<table border="0" width="100%">
				...
				<tr>
					...
					<td style="width:135px;"><input type="radio" name="data[Health][requ_status]" value="9" <?php if($this->data['Health']['requ_status'] == 9) {?> checked="checked" <?php }?> onclick="closed_statesde(this.value);" />&nbsp;&nbsp;Yes
						<input type="radio" name="data[Health][requ_status]" value="6" <?php if($this->data['Health']['requ_status'] == 6) {?> checked="checked" <?php }?> onclick="closed_statesde(this.value);" />&nbsp;&nbsp;No
					</td>
				</tr>
				<tr id="MakeDiv">
					<td>&nbsp;</td>
					<td class="boldText">Enter Message</td>
					<td><?php echo $form->textarea('Health.description', array('class'=>'input-text','rows'=>5,'cols'=>50,'style'=>'font-size:12px;','required'=>'required')); ?></td>
				</tr>
	...
	<tr id="PartialMarkClosed" class="tabs" style="display:none;">
		<td colspan="2">
		<?php if($health_data['Health']['balance_amount']==0 && $health_data['Health']['total_amount']==$health_data['Health']['recieved_amount']){	 ?>
			<?php echo $form->create(array('url'=>'/admin/samples/partial_mark_closed/'.base64_encode($this->data['Health']['id']),'id'=>'form91','name'=>'form9'));?>
			...
			<table border="0" width="100%">
				...
				<tr>
					<td style="font-weight:bold; width:10px;">1)</td>
					<td style="width:135px;" class="boldText">Closed this status</td>
					<td style="width:135px;"><input type="radio" name="data[Health][requ_status]" value="17" <?php if($this->data['Health']['requ_status'] == 17) {?> checked="checked" <?php }?> onclick="closed_statesd(this.value);" />&nbsp;&nbsp;Yes
						<input type="radio" name="data[Health][requ_status]" value="7" <?php if($this->data['Health']['requ_status'] == 7) {?> checked="checked" <?php }?> onclick="closed_statesd(this.value);" />&nbsp;&nbsp;No
					</td>
				</tr>
				<tr id="PartialMakeDiv" style="display:none;">
					<td>&nbsp;</td>
					<td class="boldText">Enter Message</td>
					<td><?php echo $form->textarea('Health.partial_description', array('class'=>'input-text','rows'=>5,'cols'=>50,'style'=>'font-size:12px;','required'=>'required')); ?></td>
				</tr>
			...
			{
				echo "Dues Are Still Pending.";
			}
			?>
		</td>
	</tr>
	
	<tr id="BpNet" class="tabs" style="display:none;">
		<td colspan="2">
			<?php echo $form->create(array('url'=>'/admin/samples/mark_closed/'.base64_encode($this->data['Health']['id']),'id'=>'form9','name'=>'form9'));?>
	...
	<?php }?>
	<tr id="ProcessRequest" class="tabs" style="display:none;">
		<td colspan="2">
	...
	<tr id="editDetails" class="tabs" style="display:none;">
		<td colspan="2">
			<table border="0" width="100%">
				<tr>
				...
				<tr>
					...
					<td>
						<input class ="input-text" name="data[Health][email]" value="<?php echo $this->data['Health']['email'];?>" id="HealthEmail" type="text" /></tr>
						<div id="msg3" style="color:#FF0000; font-size:12px;"></div>
					</td>
				</tr>
				<tr>
					<td width="15%" class="boldText">Alternate Email <font color="#FF0000">*</font></td>
					<td>
						<input class ="input-text" name="data[User][alternate_email]" value="<?php echo $this->data['Health']['alternate_email'];?>" id="HealthAlternateEmail" type="text" /></tr>
						<div id="msg3" style="color:#FF0000; font-size:12px;"></div>
					</td>
				</tr>
				<tr>
				...
				<tr id="home_collection_2" <?php if(!empty($this->data['Health']['home_collect_time']) && !empty($this->data['Health']['home_collect_date'])) {?> style="" <?php } else {?> style="display:none;" <?php }?> >
					...
					<td>
						<select name="data[Health][sample_time1]" id="HealthSampleTime1" class="input-text" disabled>
							<option value="">Select Time</option>
							<?php foreach($timelabs as $key => $val) {?>
							<option value="<?php echo $val['Timelab']['sequence'];?>" <?php if($this->data['Health']['home_collect_time'] == $val['Timelab']['name']) {?> selected="selected" <?php }?>><?php echo $val['Timelab']['name'];?></option>
							<?php }?>
						</select>
					</td>
				</tr>
	...
	<tr id="updatePayStatus" class="tabs" style="display:none;">
		<td colspan="2">
			<table border="0" width="100%">
				<tr>
				...
				<tr>
					<td width="50%">
						<table border="0" width="100%">
							...
							<tr id="wallet_name" style="display:none;">
								<td width="25%" style="font-weight:bold;">Wallet Name</td>
								<td><input type="text" name="data[Health][wallet_name]" class="input-text" style="width:150px;" id="HealthWalletName"></td>
							</tr>
							<tr id="trans_id" style="display:none;">
								<td width="25%" style="font-weight:bold;">Transaction Id</td>
								<td><input type="text" name="data[Health][trans_id]" class="input-text" style="width:150px;" id="HealthTransId"></td>
							</tr>
							...
							<tr id="auth_code" style="display:none;">
								<td width="25%" style="font-weight:bold;">Authorization Code</td>
								<td><input type="text" name="data[Health][auth_code]" class="input-text" style="width:150px;" id="HealthAuthCode"></td>
							</tr>
							<tr id="bank_name" style="display:none;">
								<td width="25%" style="font-weight:bold;">Bank Name</td>
								<td><input type="text" name="data[Health][bank_name]" class="input-text" style="width:150px;" id="HealthBankName"></td>
							</tr>
							...
							<tr class="pay_amt" style="display:none;">
								...
								<td>Rs. <?php echo $form->text('Health.pcc_amt',array('value'=>'0','class'=>'input-text','style'=>'width:100px;','onkeypress'=>'return checkpin(this)'));?></td>
							</tr>
	...
	<?php }?>
	<tr id="uploadReport" class="tabs" style="display:none;">
		<td colspan="2">
			<table border="0" width="100%">
				<tr>
...
function sendmailtopcc()
{
	...
	if(email=='')
	{
		...
		$('#mail_issue').show();
		$('#mailprocessing').hide();
		$('#mail_issue').html('Email Cannot be Empty');
		return false;
	}
	$.ajax({
		type:'POST',
...
function report_message()
{
	...
	if(phone!="")
	{
		...
		$.ajax({
			type:'POST',
			url:siteUrl+'admin/samples/send_on_sms',
			data : {
				phone : phone,
				health_id : healthId,
			},
...
function whatsapp()
{
	var phone = $('#whatsapp_number').val();
	var healthId = $('#HealthRequestId').val();
	if(phone!="")
	{
		$('#what_error').hide();
		$('#what_processing').show();
		$('#send_whatsapp').hide();
		
		$.ajax({
			type:'POST',
			url:siteUrl+'admin/samples/send_on_whatsapp',
			data : {
				phone : phone,
				health_id : healthId,
			},
...
function enableTextbox(e)
{
	...
	if(disable_status == true)
	{
		...
		document.getElementById('sample_barcode_'+id_part).required = true;
	}
	else
	{
		document.getElementById('sample_barcode_'+id_part).disabled = true;
		document.getElementById('sample_barcode_'+id_part).required = false;
...
function droplet_trigger(){
	var healthId = $('#HealthRequestId').val();
	$('#dropletupdate').hide();
	$('#dropletprocessing').show();
	$.ajax({
		type:'POST',
		url:siteUrl+'home/droplet_api',
		data : {
			id : healthId,
		},
		dataType:"json",
		success:function(result){
			...
			if(result["status"] != "failed")
			{
				$('#dropletcompleted').show();
			}
			else
				$('#dropletfailed').show();
			$('#dropletprocessing').hide();
			$('#dropletupdate').show();
		}
	});
}
...
<?php echo $html->css('admin/wickedpicker.min');?>
<?php echo $javascript->link('admin/wickedpicker.min');?>
