...
<div class="contentcontainer">
    <div class="headings altheading">
        <h2>Serviced By BP Discounting Report</h2>
    </div>
    <div class="contentbox">
        <?php echo $this->Session->flash(); ?>
        <?php echo $html->link('Home', '/admin', array('title'=>'Home')); ?>&nbsp;&#187;&nbsp;Serviced By BP Discounting Report
	<div>&nbsp;</div>
        <?php e($form->create('serviced_by', array('url'=>array('controller'=>'profit_sharing', 'action'=>'serviced_by'),'class'=>' zpFormLightgreen','id'=>'formreport'))); ?>
	...
	<div id="setOption" style="display:none;"></div>
		...
		<tr>
                    <td colspan="9">
                        <table border="0" width="100%">
                            <tr>
						<td width="30"><?php echo $form->text('ServicedBy.from_date',array('class'=>'input-Search datepicker','placeholder'=>'From Date'));?></td>
						<td width="30"><?php echo $form->text('ServicedBy.to_date',array('class'=>'input-Search datepicker','placeholder'=>'To Date'));?></td>

                                                <td width="30">
                                                        <?php $empty=false; if(count($pcc_list) > 1) $empty="Select Booked By PCC" ; ?>
							<?php e($form->select('ServicedBy.pcc_list_id1', $pcc_list, null, array('class'=>'input-Search','empty'=>$empty),null,false))?>
                                                </td>

                                                <td width="30">
                                                        <?php $empty=false; if(count($pcc_list) > 1) $empty="Select Serviced By PCC" ; ?>
							<?php e($form->select('ServicedBy.pcc_list_id', $pcc_list, null, array('class'=>'input-Search','empty'=>$empty),null,false))?>
                                                </td>
                                                
                                                
                                                
                                                <td width="30">
							<?php e($form->select('ServicedBy.agent_list_id', $agent_list, null, array('class'=>'input-Search','empty'=>'Select Agent'),null,false))?>
                                                </td>
			...
			<td colspan="9">
				<table border="0" width="100%" class="thickBorder">
					...
					<tr>
						...
						<td><?php echo 'Rs. '.$summary['booking_income_percent'].' %';?></td>
		...
		<tr>
			...
			<td class="recordRow"><?php echo !empty($rep_val['Health']['created_by'])?$pcc_list[$rep_val['Health']['created_by']]:'NPL';?></td>
